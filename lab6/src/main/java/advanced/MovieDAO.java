...
public class MovieDAO {
    ...
    public void create(Movie movie) throws SQLException {
        ...
        try (Connection connection = homework.Database.getInstance().getConnection();
    ...
    public List<Movie> findAllWithActors() throws SQLException {
        String sql = """
                SELECT m.id, m.title, m.release_date, m.duration, m.score,
                       g.id, g.name,
                       a.id, a.first_name, a.last_name, a.birth_date, a.nationality
                FROM movies m
                JOIN genres g ON m.genre_id = g.id
        ...
        try (Connection connection = Database.getInstance().getConnection();
            ...
            while (resultSet.next()) {
                ...
                if (!movieMap.containsKey(id)) {
                    Genre genre = new Genre(resultSet.getInt(6), resultSet.getString(7));
                    Movie movie = new Movie(id, resultSet.getString("title"), resultSet.getDate("release_date").toString(), resultSet.getInt("duration"), resultSet.getInt("score"), genre);
                    movieMap.put(id, movie);
                }

                int actorId = resultSet.getInt(8);
                if (actorId != 0) {
                    movieMap.get(id).addActor(new Actor(actorId, resultSet.getString("first_name"), resultSet.getString("last_name"), resultSet.getString("birth_date"), resultSet.getString("nationality")));
                }
            }
        }
